...
def upload_file(file_obj):
    ...
    try:
        s3_client.upload_fileobj(
            file_obj, settings.AWS_BUCKET_NAME, f"{UPLOAD_DIRECTORY}/{obj_name}"
        )
...
def delete_remote_file(uri):
    ...
    try:
        s3_client.delete_object(
            Bucket=settings.AWS_BUCKET_NAME, Key=f"{UPLOAD_DIRECTORY}/{uri}"
        )
    ...
    try:
        return s3_client.generate_presigned_url(
            ...
            Params={
                ...
                "Key": f"{UPLOAD_DIRECTORY}/{s3_file_name}",
            },
            ExpiresIn=3600,
        )
    except ClientError as e:
